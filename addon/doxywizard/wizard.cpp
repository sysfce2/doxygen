...
void Step1::retranslateUi()
{
  ...
  if (m_projIconLab->pixmap(Qt::ReturnByValue).isNull())
#else
...
Wizard::Wizard(const QHash<QString,Input*> &modelData, TranslationManager *translationManager, QWidget *parent) :
  QSplitter(parent), m_modelData(modelData)
{
  ...
  connect(translationManager, SIGNAL(languageChanged(QString)),
